<section>
    ...
    <form method="post" action="{{ route('ticket.update', $ticket->id) }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        ...
        <div>
            ...
            <x-textarea id="description"  name="description" class="mt-1 block w-full" rows="4" :value="old('description', $ticket->description)" required autofocus/>
            <x-input-error class="mt-2" :messages="$errors->get('description')" />
        </div>

        <div>
            @if($ticket->attachment)
                <a href="{{ '/storage/'.$ticket->attachment }}" class="text-white underline" target="_blank"><x-input-label for="exattachment" :value="__('Existing Attachment')" /></a>
            @endif
